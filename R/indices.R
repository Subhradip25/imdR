...
compute_rainfall_indices <- function(rain_raster,
     ...
     if (is.list(rain_raster) && !inherits(rain_raster, "SpatRaster")) {
          cat("Multi-year list -- processing year by year...\n")
     ...
     if (!is.null(level) && !is.null(name)) {
          ...
          rain_raster <- terra::mask(
               terra::crop(rain_raster, terra::vect(boundary)),
               terra::vect(boundary))
          cat("Computing indices for:", name, "\n\n")
     }

     dates  <- as.Date(names(rain_raster))
     years  <- unique(format(dates, "%Y"))
     n_cell <- terra::ncell(rain_raster)
     cat("Computing rainfall indices for", length(years), "year(s)...\n")
     results <- list()
...
compute_temp_indices <- function(tmax_raster,
     ...
     if (!is.null(level) && !is.null(name)) {
          boundary    <- get_boundary(level, name)
          tmax_raster <- terra::mask(terra::crop(tmax_raster, terra::vect(boundary)),
                                     terra::vect(boundary))
          tmin_raster <- terra::mask(terra::crop(tmin_raster, terra::vect(boundary)),
                                     terra::vect(boundary))
          cat("Computing temperature indices for:", name, "\n\n")
     }

     dates  <- as.Date(names(tmax_raster))
     years  <- unique(format(dates, "%Y"))
     n_cell <- terra::ncell(tmax_raster)
     cat("Computing temperature indices for", length(years), "year(s)...\n")
     results <- list()
